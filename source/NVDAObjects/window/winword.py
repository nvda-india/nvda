...
class WinWordCollectionQuicknavIterator(object):
	...
	def iterate(self):
		...
		itemCount=items.count if isinstance(items.count,(int, long)) else len(items)
		#log.io("\n"+str(itemCount)+"\n")
		isFirst=True
		for index in xrange(1,itemCount):
...
class WordDocumentTreeInterceptor(browseMode.BrowseModeDocumentTreeInterceptor):
	...
	def _iterNodesByType(self,nodeType,direction="next",pos=None):
		...
		if nodeType=="link":
			...
			return ShapeWinWordCollectionQuicknavIterator(nodeType,self,direction,rangeObj,includeCurrent).iterate()
		else:
			raise NotImplementedError
