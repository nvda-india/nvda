...
#         self.reportSmartArtSummary()

    def _get_name(self):
        if self.totalNodeCount>0:
            if self.totalNodeCount==1:
                # Translators: Indicates that there is 1 node in smartart.
                text = _( "There is 1 node in this smart art" )
            else:
                # Translators: Indicates the number of nodes in smartart.
                text = _( "There are %d nodes in this smart art" ) %(self.totalNodeCount)
        if self.wordSmartArtObject.Layout.Name:
            # Translators: Indicates the Layout Name of SmartArt.
            text += ", " + _("Smart art layout {name} ").format(name=self.wordSmartArtObject.Layout.Name)
        return text

    def script_nextNode(self, gesture):
        if self.currentNodeIndex == self.totalNodeCount-1:
            self.currentNodeIndex=0
        else:
            self.currentNodeIndex = self.currentNodeIndex + 1
        obj=WordSmartArtNode(windowHandle=self.windowHandle, wordApplicationObject=self.wordApplicationObject, wordShapeObject=self.wordShapeObject, currentNodeIndex=self.currentNodeIndex)
        eventHandler.queueEvent('gainFocus',obj)
    script_nextNode.canPropagate=True

    def script_previousNode(self, gesture):
        if self.currentNodeIndex == 0:
            self.currentNodeIndex=self.totalNodeCount-1
        else:
            self.currentNodeIndex = self.currentNodeIndex - 1
        obj=WordSmartArtNode(windowHandle=self.windowHandle, wordApplicationObject=self.wordApplicationObject, wordShapeObject=self.wordShapeObject, currentNodeIndex=self.currentNodeIndex)
        eventHandler.queueEvent('gainFocus',obj)
    ...
    __gestures={
#         "kb:leftArrow":"previousNode",
        "kb:downArrow":"previousNode",
#         "kb:rightArrow":"nextNode",
        "kb:upArrow":"nextNode",
...
class WordSmartArtNode(WordSmartArt):
    def __init__(self,windowHandle, wordApplicationObject, wordShapeObject, currentNodeIndex):
        super(WordSmartArtNode, self).__init__(windowHandle=windowHandle, wordApplicationObject=wordApplicationObject, wordShapeObject=wordShapeObject, currentNodeIndex=currentNodeIndex)

#     def event_gainFocus(self):
# #         self.wordSmartArtObject.AllNodes(self.currentNodeIndex+1).Select()
#         self.reportNodeSummary()

    def _get_name(self):
        text= _( "node {index}, ".format(index=self.currentNodeIndex+1))
        if (self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Shapes.AutoShapeType==msoShapeMixed) and (self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Shapes.Count > 1):
            for j in xrange(1, self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Shapes.Count + 1):
                text += _("shape {index} {shapeString}, ".format(index=j, shapeString=msoShapeTypeLabels.get(self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Shapes(j).AutoShapeType)) )
        else:
            text += _("shape {shapeString}, ".format(shapeString=msoShapeTypeLabels.get(self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Shapes.AutoShapeType)) )
        try:
            if self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).TextFrame2.TextRange.Text:
                text += _( "text {textString}. ").format(textString=self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).TextFrame2.TextRange.Text)
        except:
            pass
        childNodesCount=self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Nodes.Count
        if childNodesCount == 1:
        	text += _("It has one child node")
        	text += _("child node {index}, shape {shapeString}, ".format(index=1, shapeString=msoShapeTypeLabels.get(self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Nodes(1).Shapes.AutoShapeType)))
        	try:
        		if self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Nodes(1).TextFrame2.TextRange.Text:
        			text += _( "text {textString} ").format(textString=self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Nodes(1).TextFrame2.TextRange.Text)
        	except:
        		pass
        elif childNodesCount > 1:
            text += _("It has {childCount} child nodes: ".format(childCount=childNodesCount))
            for i in xrange(1,childNodesCount+1):
                text += _("child node {index}, shape {shapeString}, ".format(index=i, shapeString=msoShapeTypeLabels.get(self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Nodes(i).Shapes.AutoShapeType)))
                try:
                    if self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Nodes(i).TextFrame2.TextRange.Text:
                        text += _( "text {textString} ").format(textString=self.wordSmartArtObject.Nodes(self.currentNodeIndex+1).Nodes(i).TextFrame2.TextRange.Text)
                except:
                    pass
        return text
